...
public class SubmissionManager {
    ...
    private static void asyncSubmitWork(CommandSourceStack src, FreezeReport report, String markdown,
        ...
        src.sendSuccess(() -> CommandFeedback.info("- Build ID: " + StutterAnalyzerFabric.BUILD_ID), false);
        ...
        CompletableFuture.runAsync(() -> {
            try {
                HttpRequest req = HttpRequest.newBuilder()
                    .uri(URI.create(endpoint))
                    .header("Content-Type", "application/json")
                    .header("User-Agent", "StutterAnalyzer/" + StutterAnalyzerFabric.MOD_VERSION + " Minecraft/1.20.4 (minimal-test)")
                    .timeout(Duration.ofSeconds(timeoutSec))
                    .POST(HttpRequest.BodyPublishers.ofString(payload, StandardCharsets.UTF_8))
                    .build();
                HttpResponse<String> resp = HTTP_CLIENT.send(req, HttpResponse.BodyHandlers.ofString());
    ...
    public static int submitCheckStatus(CommandSourceStack src, String reportId) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                StutterAnalyzerFabric.LOGGER.warn("[SA] Report status check failed: {}", e.getMessage());
